...
const { MongoDBCollection } = require('@ibilux/realm-node/lib/mongo-client.cjs');
const { cleanArguments, promisify } = require('@ibilux/realm-node/lib/utils.cjs');
...
const instanceMethods = {
  ...
  async _callFunctionOnService(name, serviceName, ...args) {
    ...
    const result = await promisify(cb =>
      this._callFunction(name, stringifiedArgs, serviceName, cb)
    );
