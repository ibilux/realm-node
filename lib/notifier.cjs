...
const Worker = require('@ibilux/realm-node/lib/worker.cjs');
...
class Listener {
  constructor(Sync, config) {
    this.notifier = Sync._createNotifier(
      config.serverUrl,
      config.adminUser,
      (event, a1, a2) => this[event](a1, a2),
      config.sslConfiguration,
      config.listenerDirectory
    );
    this.initPromises = [];
    this.callbacks = [];
  }
  ...
  add(regexStr, event, fn) {
    ...
    if (typeof fn === 'function') {
      ...
      throw new Error(
        `Invalid arguments: must supply either event name and callback function or a Worker, got (${event}, ${fn})`
      );
    }
  ...
  _notifyDownloadComplete() {
    ...
    if (this.err) {
      for (let [, reject] of this.initPromises) {
        setImmediate(() => reject(this.err));
      }
    } else {
      for (let [resolve] of this.initPromises) {
...
module.exports = function (Realm) {
  Realm.App.Sync.getListenerDirectory = getListenerDirectory;
  Realm.App.Sync.setListenerDirectory = setListenerDirectory;
  Realm.App.Sync.addListener = addListener.bind(Realm.App.Sync);
  Realm.App.Sync.removeListener = removeListener;
  Realm.App.Sync.removeAllListeners = removeAllListeners;
  Realm.App.Sync.localListenerRealms = localListenerRealms.bind(Realm.App.Sync, Realm);
};
