...
const { promisify } = require('realm-node/lib/utils.cjs');
...
const instanceMethods = {
  registerUser(...args) {
    const { argsObject } = handleDeprecatedPositionalArgs(args, 'registerUser', [
      'email',
      'password'
    ]);
  ...
  confirmUser(...args) {
    const { argsObject } = handleDeprecatedPositionalArgs(args, 'confirmUser', [
      'token',
      'tokenId'
    ]);
  ...
  resendConfirmationEmail(...args) {
    const { argsObject } = handleDeprecatedPositionalArgs(args, 'resendConfirmationEmail', [
      'email'
    ]);
  ...
  retryCustomConfirmation(...args) {
    const { argsObject } = handleDeprecatedPositionalArgs(args, 'retryCustomConfirmation', [
      'email'
    ]);
  ...
  sendResetPasswordEmail(...args) {
    const { argsObject } = handleDeprecatedPositionalArgs(args, 'sendResetPasswordEmail', [
      'email'
    ]);
  ...
  resetPassword(...args) {
    const { argsObject } = handleDeprecatedPositionalArgs(args, 'resetPassword', [
      'password',
      'token',
      'tokenId'
    ]);
